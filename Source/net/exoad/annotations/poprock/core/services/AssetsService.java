package net.exoad.annotations.poprock.core.services;
 ...
 * that is bundled with the Poprock library and program. It can be used to
 ...
 * required asset where it will first try to search in the pre-baked folder,
 * then outside the pre-baked folder. If both of those fail for some reason, a
 * final {@code null} is returned which is highly unlikely if the asset is
 * actually present.
 * <p>
 * <em>There are plans to migrate to using {@link java.util.Optional}'s to
 * allow for better catching of nonexistent assets, but that just introduces
 * overhead as 99.9% of the time, the asset should be there.</em>
 *
 * @author Jack Meng
 */
...
public final class AssetsService
{
	...
	/**
	 * Set to where this service should search in and append paths after. Rarely
	 ...
	 * instead this function is called when we want to modify the image after,
	 * such as scaling, filtering, etc..
	 * <p>
	 * <strong>Example Usage</strong>
	 * <p>
	 * <blockquote><pre>

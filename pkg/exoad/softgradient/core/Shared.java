 ...
 * atomicity operations, such as modifying a non-final variable inside a lambda.
 * <p>
 ...
 * does it even achieve
 * such a goal; instead, it is focused on making sure multiple instances of an
 * object that can hold
 * the same value don't exist at the same time. For example, in concurrency,
 * Shared objects will help
 * to maintain atomicity and mitigate race conditions.
 * <p>
 * The Shared Class itself does not hold
 * the true
 * value of the object, instead a singular ID or numerical pointer to the value
 * in the pool. Furthermore, the usage of this class is very very simple and
 * acts just like a normal Wrapper class that
 * just exists to get rid of the "modification of non-final variable in a
 * lambda" error thrown by Java. Just look at {@link #setValue(Object)} and
 * {@link #getValue()}
 * they can't be anymore easier! <em>All the internal complexity are hidden
 * away</em>
 * <p>
...
{
   static HashMap<Long,Object> OBJECTS=new HashMap<>();

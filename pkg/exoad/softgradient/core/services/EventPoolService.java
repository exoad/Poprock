 ...
 * both the library
 * and app for dispatching events.</strong> It is structured using a Map making
 * it so
 * that listeners can subscribe to a specific payload type (a class/record that
 * implements {@link pkg.exoad.softgradient.core.services.EventPayload}).
 * <p>
 * <h3>Events can be dispatched by anyone</h3>, which is a downside to
 * this method, but
 * it proves robust for the most part in terms of ease of use. Especially
 * because there are no special
 * permissions and other shenanigans that goes into just dispatching a simple
 * event.
 * <p>
 * <h3>Listeners must know their EventPool</h3>, this is the only
 * measure that goes
 * into categorizing data passing through. Listeners must know the pool id
 * {@link #registerEventPool(int)} in order to subscribe to it.
 * <p>
 * <h3>Event Pools are ephemeral</h3>. If you are using SoftGradient as
 * a library and never call
 * {@link pkg.exoad.softgradient.AppEntry#main(String[])}, your
 * {@code 1} event pool might not be the same as the one found in the default
 * SoftGradient app.
 * For example, in the SoftGradient App, event pool {@code 1} represents a
 ...
 * bad. Especially
 * since there could be credentials being
 * passed around, but look, this app is designed for simply creating gradients
 * not being the next solution to fixing some security pandemic.
 * <p>
 * <h3>Listeners must know what they want can or cannot exist</h3>,
 * meaning something that wants
 * to listen to anything in the event pool must know that the event they want to
 * subscribe has to exist first. {@link #isRegisteredPayloadID(Class)}
 *
 * <blockquote><pre>
 * EventPool
 * .getPool(1)
 ...
 * </pre></blockquote>
 *
 * <em>The above code snipper throws an error is because {@code FooEventPayload}
 * has not been registered to the EventPool by someone
 * {@link #registerEvent(Class, Object)}
 * <p>
...
{
      protected EventPoolService()
      {
      }

      /**
       * This is a "marker" interface for all EventPayloads that want to get
       * added to
       * any kind of
       * EventPool.
       * <p>
       * <strong>Why?</strong> This way EventPool knows that the programmer has
       * acknowledged that the class used a payload
       * interface will be used properly. (AKA following MVC patterns)
       *
       * @author Jack Meng
       *
       */
       ...
       * requested pool is not found!</em></strong>
       *
      ...
      /**
       * Returns all of the payload types within an existing event
       * payload.
       * <p>
...

      {
            ...
            HashMap</*id of the event*/Class<? extends EventPayload/*must extend marker interface*/>,Pair</*all the listeners*/Stack<Runnable>,/*the currently held result of this payload*/Object>> payloads=new HashMap<>();
            ...
            /**
             * Returns the currently cached value of this payload event type
             * (can be
             * null).
             *
            ...
            /**
             * Dry runs all of the listeners under an event payload. This should
             * be
             * used sparingly as the currently cached value could be unexpected
             * or
             ...
             * @param id identifier of the payload
             */
            public final synchronized <T extends EventPayload> void ping(
                  Class<T> id
            ...
            public <T extends EventPayload> boolean isRegisteredPayloadID(
                  Class<T> id
            )
            {
